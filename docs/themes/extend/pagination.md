...
> 上述模板参数对应 `Typecho\Widget\Helper\PageNavigator\Box`，其支持的键为 `itemTag`、`textTag`、`currentClass`、`prevClass`、`nextClass`，加上 `pageNav()` 自身使用的 `wrapTag`、`wrapClass`。旧文档中提到的 `itemClass` 目前已不再支持。
...
> 当前页码通过 `$this->_currentPage` 获取（由 `Widget\Archive::____currentPage()` 提供）。旧文档中写的 `$this->currentPage` 在当前版本无法取得页码，请使用 `$this->_currentPage`。
