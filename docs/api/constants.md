...
### 后台路径自定义
后台路径自定义，用于修改程序后台地址，提高安全性。
**typecho** 默认后台地址是 **admin**，如果我们想改成 **typecho-admin**，那么首先登录服务器把 **admin** 文件夹名字改成 **typecho-admin**。
...
用法
在 `config.inc.php` 最后面插入 `define('__TYPECHO_SECURE__', true);`。

> 注意：旧版本曾使用拼写错误的常量名 `__TYPECHO_SECUER__`，当前版本已修正为 `__TYPECHO_SECURE__`（见 `Typecho\Request::isSecure()`），请使用正确拼写。
...
> 提示：程序会把邮箱的 md5 直接拼接到该常量之后，因此常量值应以 `/` 结尾。

### 自定义上传附件目录

可以单独定义一个不在 typecho 下的附件上传地址
存储到这里有什么用呢？
在 `config.inc.php` 最后面插入
...
你可以给他单独一个反向代理，就是 cdn，七牛，又拍云等 cdn 可以访问到的地址，然后通过反向代理把你这个路径抓取的，抓取过去后，你那前面的展示地址就可以换成你的 cdn 地址了，比如上述代码中 `https://xxxxx.upyun.com`。

> 另外还可以用 `__TYPECHO_UPLOAD_ROOT_DIR__` 指定上传文件的物理根目录；`__TYPECHO_UPLOAD_DIR__` 指定相对存储目录。
