...
> `threadedComments($comments, $options)` 是系统约定的自定义回调函数名：`Widget\Comments\Archive::listComments()` 在输出每条评论前会检测该函数是否存在，存在则调用它，并把当前评论对象和输出选项 `$options` 传入。
> 评论层级属性为 `levels`（旧文档中的 `_levels` 已不再有效）。
...
?>">
```
其中，替换ID这里，还需要判断当前评论是父级评论还是子级评论，且判断评论 ID 的奇偶数等。
...
<div class="comment-children">
    <?php $comments->threadedComments(); ?> //嵌套评论所有内容
</div>
<?php } ?> //是否嵌套评论判断结束
```
...
### 五、相关变量及说明
```php
<?php $comments->gravatar(40); ?> //头像，参数为：尺寸、默认头像、是否输出高清(2x/3x) srcset
