...
> 说明：当前版本后台已内置完整的站点备份功能（`admin/backup.php` + `Widget\Backup`，备份文件默认保存在 `usr/backups`），但那是针对整个站点的备份。下面介绍的是“模板设置数据”的单独备份/还原。

### 代码
在 `themeConfig($form)` 函数里添加

```php
$str1 = explode('/themes/', \Utils\Helper::options()->themeUrl);
$str2 = explode('/', $str1[1]);
...
if (isset($_POST['type'])) {
    if ($_POST["type"] == "备份模板设置数据") {
        if ($db->fetchRow($db->select()->from('table.options')->where('name = ?', 'theme:' . $name . 'bf'))) {
            ...
            ?>
            <a href="<?php \Utils\Helper::options()->adminUrl('options-theme.php'); ?>">这里</a></div>
            <script language="JavaScript">window.setTimeout("location=\'<?php \Utils\Helper::options()->adminUrl('options-theme.php'); ?>\'", 2500);</script>
            <?php
        } else {
            if ($ysj) {
    ...
    if ($_POST["type"] == "还原模板设置数据") {
        if ($db->fetchRow($db->select()->from('table.options')->where('name = ?', 'theme:' . $name . 'bf'))) {
            ...
            ?>
            <a href="<?php \Utils\Helper::options()->adminUrl('options-theme.php'); ?>">这里</a></div>
            <script language="JavaScript">window.setTimeout("location=\'<?php \Utils\Helper::options()->adminUrl('options-theme.php'); ?>\'", 2000);</script>
            <?php
        } else {
...
**备份**
当用户点击**备份**时，先判断是否已经存在备份，如果不存在就插入一条新的数据，数据name为 `主题名bf`，value为模板原本的数据。此时就存在了一条备份数据。
...
> 说明：`Helper::options()` 也可以继续使用（`Helper` 是 `\Utils\Helper` 的别名）；`Typecho_Db` 同理映射到 `\Typecho\Db`。当前版本 `themeConfig($form)` 依然是主题设置入口，代码可直接使用。
