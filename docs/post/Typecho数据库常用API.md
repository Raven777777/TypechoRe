...
在Typecho插件开发过程中，往往需要创建自己的表。上文提到`Typecho\Db`类中的`query`函数，可用于执行所有`sql`语句，因此我们使用`query()`来进行表的创建、修改或者删除。
```php
$db = \Typecho\Db::get();
$prefix = $db->getPrefix();
$db->query('create table ' . $prefix . 'metas xxxxx');
```
注意，使用`query`方式创建表的时候，需要在表名前手动添加`$prefix`前缀，否则在后面的使用过程中会造成困惑。
...
举例：
a. `$db->select()->from('table.metas');`将生成`SELECT * FROM typecho_metas`，其中`typecho_`是表前缀；
b. 而`$db->select()->from('metas');`将生成`SELECT * FROM metas`，注意这里没有了表前缀。
...
echo $query; //SELECT `mid` , `name` FROM typecho_metas WHERE (`mid` = '2' )
```
...
**对查询结果进行排序**
在`Typecho`中，使用`order()`函数和`\Typecho\Db::SORT_DESC`指定查询结果的排序方式。
```php
$query = $db->select('mid', 'name')->from('table.metas')->order('mid', \Typecho\Db::SORT_DESC);
echo $query;//SELECT `mid` , `name` FROM typecho_metas ORDER BY `mid` DESC
//Tips: \Typecho\Db::SORT_ASC 表示升序排序，\Typecho\Db::SORT_DESC 表示降序排序
```
### 联合查询
...
$query = $db->select()
    ->from('table.contents')
    ->join('table.comments', 'table.contents.cid = table.comments.cid', \Typecho\Db::LEFT_JOIN)
    ->where('table.contents.type = ?', 'post');
echo $query;
//SELECT * FROM typecho_contents LEFT JOIN typecho_comments ON typecho_contents.`cid` = typecho_comments.`cid` WHERE (typecho_contents.`type` = 'post' )
```
...
echo $update;//UPDATE typecho_metas SET `name` = 'case_in_cn' WHERE (`mid`='6' )
//执行后，返回受影响的行数。
...
**查看查询语句**
在`Typecho`调试过程中，打印`sql`语句往往是很有帮助的。`Query` 对象实现了 `__toString()`，因此直接 `echo $query` 即可输出构建好的 SQL：
```php
$select = $db->select()->from('table.metas');
echo $select;
```

> 旧写法 `Typecho_Db::get()`、`Typecho_Db::SORT_DESC`、`Typecho_Db::LEFT_JOIN` 等依然可用，会被自动映射到 `\Typecho\Db` 及其常量。
